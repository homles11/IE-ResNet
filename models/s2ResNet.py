...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        if self.stride != 1 or self.in_channels != self.out_channels * BottleNeck.expansion:
            return self.residual_function(x) + self.shortcut(x)
        else:
            x = self.residual_function(x) + x
...
class BottleNeck(nn.Module):
    ...
    def forward(self, x):
        if stride != 1 or in_channels != out_channels * BottleNeck.expansion:
            ...
            return self.residual_function(self.residual_function(x) + x)+x
